...
figure
    freqz(h_interpolador)
    %title("Diagrama de bode del nuevo muestreo");

%aumento la cantidad de muestras de la señal original
datos = resample(datos, expansor, decimador , h_interpolador); 
...
%     legend('Señal original', 'Señal preprocesada');
%     grid on
%     hold off
figure 
    hold on
    ...
    xlim([xmin xmax]);
    ylabel('Amplitud[mV]');xlabel('Tiempo[s]');
    legend('Señal original','Señal preprocesada','Qrs Detection');
    grid on
    hold off
